...
module.exports = function(modulename, opts) {
  ...
  const _M = { // short for _Module
    ...
    // If you pass a second parameter, it will override the default schema output
    // for any properties with the keys from the one passed in the optiona_overrides object.
    // To be clear, the optional_overrides is not a schema, it's just an object with the
    // vocab terms that you want to override as keys (i.e. its the properties object of a schema).
    vocabToSchema: function(terms_array, optional_overrides) {
      optional_overrides = optional_overrides || {};
      // First, get the basic schema with the vocab terms as properties:
      const ret = { properties: _M.vocabToProperties(terms_array) };
      // Next, replace any vocab terms that we wanted to override:
      _.each(optional_overrides, (term, schema) => {
        ret.properties[term] = schema;
      });
      return ret;
    },
    ...
    override: function(term_or_schema, merge, opts) {
      ...
      // Note that mergeWith recurses downward through all keys, not just top-level ones
    ...
    vocab: function (term, opts) {
      ...
      if (typeof _M._vocab[term] === 'undefined') { 
        ...
        throw new errors.UnknownVocabularyTerm('The term "'+term+'" is not a known vocabulary term in the module '+modulename);
      }
