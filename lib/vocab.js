...
function propertySchemaToProperties(main_schema, opts) {
  opts = opts || {};
  ...
  if (arr) {
    ...
    _.each(arr, function(individual_propertySchema) {
      propertySchemaToProperties(main_schema, {
  ...
  if (prop_schema.known) {
    ...
    _.each(prop_schema.known, function(key) {
      ...
      if (vocab(key, { exists: true })) {
        main_schema.properties[key] = vocab(key);
      }
...
function propertiesToPropertySchema(main_schema) {
  main_schema = _.cloneDeep(main_schema);
  ...
  // need that.
  main_schema.propertySchema = enumSchema(_.keys(main_schema.properties));
  return main_schema;
}
...
// as needed to prevent accidentally changing them.  You can have opts of:
// opts:
...
function vocab(term, opts) {
  opts = opts || {};
  if (typeof _vocab[term] === 'undefined') { 
  ...
  var ret = _.cloneDeep(_vocab[term]);
  if (opts.merge) {
    ...
    if (opts.merge.propertySchema || opts.merge.propertySchemaDefault) {
      ret = propertySchemaToProperties(ret);
    }
  }
...
function sameAs(term, merge) {
  return vocab(term, { merge: merge });
};
