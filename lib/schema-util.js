...
var vocab = require('../formats/oada-vocab');
...
// use this function for convenience.  opts can be:
// opts:
// - required: array to set as required
function vocabTermsToSchema(terms_array, opts) {
  opts = opts || {};
  return {
    required: opts.required || [], additionalProperties: true,
    propertySchema: enumSchema(terms_array),
    properties: _.zipObject(terms_array, _.map(terms_array, Vocab))
  };
};
...
function addVocabAsProperties(schema, terms_array) {
  ...
  // - first argument to oadaSchema is a markdown-formatted description of that _type 
  //   resource.  Use it!
...
function oadaSchema(schema) {
    ...
    if (!props.context) {
      throw new InvalidOadaSchemaError('you must provide a schema with a context property, even if empty');
    }
    // Second, fill in any easy missing items:
    schema.id = schema.id || 'oada-formats://'+schema.pro_type;
    // require _type and context
    schema.required = _.union(schema.required || [], [ '_type', 'context' ]);
    ...
    _.merge(props, _.zipObject(
      ...
      _.map(schema.indexing, function(index) {
        return Vocab(index, {
          ...
          merge: { propertySchemaDefault: { _type: props._type.enum[0] } },
        });
      })
    ));
